...
int main()
{
	...
	while (loop)
	{
		...
		while (random_correct_answer[j])
		{
			while (input[i])
			{
				if (input[i] == random_correct_answer[j])
				{
					printf("\x1b[1;32m%c\033[0m", random_correct_answer[j] - 32);
					hold[i] = input[i];
				}
				else
				{
					...
					if (!random_correct_answer[j])
						printf("\x1b[0;37m%c\033[0m", input[i] - 32);  // print black
					j = i;
				}
				i++;
				j++;
